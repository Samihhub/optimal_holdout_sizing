...
nobs_boot <- 500         # Number of observations to resample in each bootstrap iter
...
num_probs <- 10 # Number of probabilities to scan
...
costs <- matrix(nrow = boots, ncol = num_sizes)


# Matrix with the number of deaths per epoch and holdout size
deaths_inter <- deaths_ho <- matrix(nrow = boots, ncol = num_sizes)
old_progress <- 0
avg_disease <- matrix(nrow = boots, ncol = num_sizes)
...
for (i in 1:num_sizes) {
  for (b in 1:boots) {
    ...
    avg_disease[b, i] <- sum(Y)
    
    progress <- 100 * (((i - 1) * boots) + b - 1) / (num_sizes * boots)
    ...
    deaths_inter[b, i] <- sum(data_interv$Y == 1 & model_resp != 1)
    deaths_ho[b, i] <- sum(data_hold$Y == 1 & dr_pred != 1)
...
deaths_sd <- apply(deaths_per_frac, 2, sd)

avg_deaths <- colSums(deaths_per_frac) / boots

plot(frac_ho, avg_deaths,
#plot(costs, deaths_inter,
     pch = 16,
     ylab = "Deaths",
     xlab = "Holdout set size",
     ylim = c(min(avg_deaths - deaths_sd), max(avg_deaths + deaths_sd)))
arrows(frac_ho, 
       avg_deaths - deaths_sd,
       frac_ho,
       avg_deaths + deaths_sd, 
       length=0.05, angle=90, code=3)
#points(frac_ho, deaths_inter, col = 2, pch = 16)
